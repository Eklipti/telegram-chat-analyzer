...
def build_social_graph(input_json: Path, out_dir: Path) -> None:
    ...
    for mentioned, count in mention_counter.most_common(15):
        ...
        mentions_top.append({"mentioned": mentioned, "username": username, "count": int(count)})
    ...
    for user_id, count in quotability_counter.most_common(10):
        quotability_top.append(
            {"user_id": user_id, "username": name_by_id.get(user_id, user_id), "replies_received": int(count)}
        )
    ...
    for domain, count in domain_counter.most_common(15):
        domains_top.append({"domain": domain, "count": int(count)})
    ...
    social_graph_data = {
        ...
        "reply_matrix": {"description": "Кто кому чаще отвечает (таблица пар)", "top_pairs": reply_matrix_top},
        "quotability_index": {"description": "На чьи сообщения чаще всего отвечают", "top_quoted": quotability_top},
        "voice_lovers": {
        ...
        "vocabulary_diversity": {
            "description": (
                ...
                "Среднее разнообразие в скользящем окне 1000 слов. Учитывается лемматизация "
                "и фильтрация стоп-слов (минимум 1000 сообщений и 1000 слов)"
            ),
            "top_users": vocabulary_top_list,
        },
        "reaction_speed": {
            "description": (
                "Медианное время ответа на сообщения через reply для каждого пользователя "
                "(топ-10 самых быстрых, минимум 1000 сообщений)"
            ),
            "top_users": reaction_speed_top,
        },
        "owls_vs_larks": {
            "description": (
                "Категоризация пользователей по времени активности. "
                "Совы (ночь 01:00-06:00 > 30%), "
                "Жаворонки (день 09:00-17:00 > 50%), минимум 1000 сообщений"
            ),
            "users": owls_vs_larks,
        },
        "self_censorship": {
            "description": (
                "Индекс самоцензуры - кто чаще редактирует свои сообщения после отправки "
                "(топ-10, минимум 1000 сообщений)"
            ),
